...
const ReviewBreakTimerView: React.FC<Props> = ({
  duration,
  pendingSubtopics,
  nextSubject,
  onComplete,
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-white dark:bg-gray-950 flex flex-col items-center justify-between p-8 animate-in fade-in zoom-in duration-300">
      ...
      <div className="flex flex-col items-center gap-6 w-full max-w-xs">
        <div className="relative flex items-center justify-center w-64 h-64">
          <svg className="w-full h-full -rotate-90 transform" viewBox="0 0 100 100">
            <circle className="stroke-current text-gray-100 dark:text-gray-900" strokeWidth="4" fill="transparent" r="45" cx="50" cy="50" />
            <circle
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-1">
            ...
            <span className="text-[10px] font-bold text-amber-500 uppercase tracking-widest">revisão</span>
          </div>
        </div>

        {/* Subtopics to review */}
        {pendingSubtopics.length > 0 && (
          <div className="w-full bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-900/30 rounded-2xl p-4 space-y-2">
            <p className="text-[9px] font-black text-amber-600 dark:text-amber-500 uppercase tracking-widest">
              Revisar agora
            </p>
            <div className="flex flex-wrap gap-1.5">
              {pendingSubtopics.map((s, i) => (
                <span
                  key={i}
                  className="px-2.5 py-1 bg-amber-100 dark:bg-amber-900/30 rounded-full text-[11px] font-bold text-amber-800 dark:text-amber-300"
                >
                  {s}
                </span>
              ))}
        ...
        {nextSubject && (
          <div className="flex items-center gap-2.5 px-4 py-2.5 rounded-full bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-800">
            <span className="text-[10px] font-bold text-gray-400 dark:text-gray-600 uppercase tracking-widest">A seguir</span>
      ...
      <div className="w-full max-w-xs space-y-3 pb-12">
        <button
          onClick={onSkip}
          className="w-full py-5 rounded-3xl font-black text-xs uppercase tracking-[0.2em] bg-amber-500 text-white shadow-xl shadow-amber-500/20 active:scale-95 transition-all flex items-center justify-center gap-3"
        >
      ...
      <div className="absolute -bottom-24 -right-24 w-64 h-64 rounded-full opacity-5 blur-3xl pointer-events-none bg-amber-500" />
    </div>
  );
};

export default ReviewBreakTimerView;
