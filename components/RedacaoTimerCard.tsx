...
export interface RedacaoTimerHandle {
  /** ms elapsed (running + accumulated), accurate up to last render */
  getElapsedMs: () => number;
  /** force pause (used when finishing the essay) */
  pause: () => void;
}

interface Props {
  totalMs: number;
  /** if true the timer starts running on mount (no Iniciar needed) */
  autoStart?: boolean;
...
const RedacaoTimerCard = forwardRef<RedacaoTimerHandle, Props>(({
  totalMs,
  autoStart = false,
  onTimeUp,
  onRunningChange,
  ...
  const [runningSince, setRunningSince] = useState<number | null>(autoStart ? Date.now() : null);
  ...
  useImperativeHandle(ref, () => ({
    getElapsedMs: () => Math.min(totalMs, liveElapsed),
    pause: () => {
      if (runningSince) {
  ...
  }), [liveElapsed, runningSince, totalMs]);
  ...
  }, [isRunning, remaining, timeUp, totalMs, onTimeUp]);
  ...
  const timerColor = remaining <= 10 * 60 * 1000 ? '#ef4444'
    ...
    : '#d97706';
  const progressPct = (liveElapsed / totalMs) * 100;
  ...
  return (
    <>
      ...
      {/* Floating mini-timer (mobile only, while running) */}
      {showFloatingPill && isRunning && createPortal(
        <button
          onClick={pause}
      ...
      {/* Time-up modal — only shown if parent doesn't intercept */}
      {timeUp && !suppressTimeUpModal && !dismissedTimeUp && createPortal(
...
RedacaoTimerCard.displayName = 'RedacaoTimerCard';

export default RedacaoTimerCard;
